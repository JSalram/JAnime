...
let wCodeGeass = localStorage.getItem("wCodeGeass");
...
function printChapters() {
    ...
    for (let i = 1; i <= chaps.length; i++) {
        ...
        let chapter = chaps[i-1];

        li.classList.add("list-group-item");
        row.classList.add("row");
        a.href = `https://mega.nz/embed/${chapter}/`;
        a.target = "_blank";
...
function toggleWatch(i, element) {
    ...
    localStorage.setItem("wCodeGeass", wCodeGeass);
...
function setWatched() {
    ...
    xhttp.open("GET", "../php/watched.php?anime=CodeGeass&user=" + user + "&w=" + wCodeGeass, true);
    xhttp.send();
}
